class FiscalPeriodsController < ApplicationController
	active_scaffold :fiscal_periods do |config|
		config.actions = [:list, :create, :delete, :update]
		config.columns = [:time_frame, :accounts]
		config.columns[:time_frame].label = 'Time frame'
		config.create.columns = [:startdate, :enddate]
		config.update.columns = [:startdate, :enddate]
	end
#   def index
#     list
#     render :action => 'list'
